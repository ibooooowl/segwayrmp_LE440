...
{
    segway::Logger::SetMinLogLevel(segway::LogLevel::INFO);
    segway::Logger::AddLogFile(std::string("rmp_interface_440le.log"));
    segway::Logger::AddOutputStream(std::cout);
}
...
void Rmp440LE::Initialize()
{
  ...
  try 
  {
    ...
    // Reset wheel encoders
     SEGWAY_LOG(segway::LogLevel::INFO, "Reset wheel encoder: " << segway::RESET_ALL_POSITION_DATA);
    ...
    {
        SEGWAY_LOG(segway::LogLevel::INFO,"Reset wheel encoder:" << segway::RESET_ALL_POSITION_DATA <<  "---ok");
      m_Rmp440LEInterface.SetConfiguration(segway::RmpConfigurationCommandSet::SET_AUDIO_COMMAND, static_cast<uint32_t>(segway::MOTOR_AUDIO_TEST_SWEEP));
    }
    ...
    // Set the max speeds
//      SEGWAY_LOG(segway::LogLevel::INFO,"Set the max speeds: " << segway::RmpConfigurationCommandSet::SET_MAXIMUM_VELOCITY);
//      SEGWAY_LOG(segway::LogLevel::INFO,"Set the max turn rate: " << segway::RmpConfigurationCommandSet::SET_MAXIMUM_TURN_RATE);
    ...
    {
//        SEGWAY_LOG(segway::LogLevel::INFO,"Set the max speeds:" << segway::RmpConfigurationCommandSet::SET_MAXIMUM_VELOCITY<< "---ok");
//        SEGWAY_LOG(segway::LogLevel::INFO,"Set the max turn rate:"<< segway::RmpConfigurationCommandSet::SET_MAXIMUM_TURN_RATE <<"---ok");
      m_Rmp440LEInterface.SetConfiguration(segway::RmpConfigurationCommandSet::SET_AUDIO_COMMAND, static_cast<uint32_t>(segway::MOTOR_AUDIO_TEST_SWEEP));
...
//      SEGWAY_LOG(segway::LogLevel::INFO,"Set update Frequency:" << updateFrequency);
    ros::Rate rate(updateFrequency);
//      SEGWAY_LOG(segway::LogLevel::INFO,"Set update Frequency:" << updateFrequency<< " ---ok");
...
//    SEGWAY_LOG(segway::LogLevel::INFO,"max velocity:" << maximumVelocity << ", max turn rate:" << maximumTurnRate);
//    SEGWAY_LOG(segway::LogLevel::INFO,"velocity:" << rpVelocityCommand->twist.linear.x << ", max turn rate:" << rpVelocityCommand->twist.angular.z);
